...
public:
    Node* connect(Node* root) {
        ...
        de.emplace_back(pair<Node*,int>(root,0));
        while(de.empty()==false){
            auto tmp=de.front();
            now.emplace_back(pair<Node*,int>(tmp.first,tmp.second));
            de.pop_front();
            if(tmp.first->left!=nullptr){
                de.emplace_back(pair<Node*,int>(tmp.first->left,tmp.second+1));
            }
            if(tmp.first->right!=nullptr){
                de.emplace_back(pair<Node*,int>(tmp.first->right,tmp.second+1));
            }
        }
